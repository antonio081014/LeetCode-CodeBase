class Solution {
    struct Candidate: Comparable, Hashable {
        static func < (lhs: Solution.Candidate, rhs: Solution.Candidate) -> Bool {
    ...
    func reorganizeString(_ s: String) -> String {
        var occurDict = [Character : Int]()
        for c in s {
            occurDict[c, default: 0] += 1
        }
        
        var pq = PriorityQueue<Candidate>(sortingComparator: >)
        for (cand, count) in occurDict {
            pq.enqueue(element: Candidate(content: cand, occurence: count))
        }
        var backupQuque = [Candidate]()
        var result = ""
        while pq.isEmpty == false {
            var element = pq.dequeue()!
            if result.last == element.content {
                backupQuque.append(element)
                continue
            }
            ...
            for item in backupQuque {
                pq.enqueue(element: item)
            }
            backupQuque = []
        }
